...
public class Fire extends AutoSection {
    ...
    public void update() {
        ...
        groundDistance = camera.getGroundDistance(FieldMap.POWERPOWER_HEIGHT);
        angle = Turret.findDesiredAngle(groundDistance, FieldMap.POWERPOWER_HEIGHT, Constants.LAUNCHER_DEFAULT_VELOCITY);
        clampAngle();
    ...
    public void clampAngle() {
        ...
        if (angle == Constants.HOOD_MIN_POSITION || angle == Constants.HOOD_MAX_POSITION) {
            velocity = Turret.findDesiredVelocity(groundDistance, FieldMap.POWERPOWER_HEIGHT, angle);
        }
    }
